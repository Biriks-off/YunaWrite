...
static void sp_help()
{
    ...
    std::cout << "   -opin(CODE)             在 ";
    show_osname();
    std::cout << " 环境中运行 .sh/.bat/.command 文件" << std::endl;
    std::cout << "C/C++:" << std::endl;
    std::cout << "请使用 GCC" << std::endl;
    std::cout << "   -cpp(code)              生成 .bat/.command/.sh 文件(cpp)" << std::endl;
    std::cout << "   -cbat(code)             生成 .bat/.command/.sh 文件(c)" << std::endl;
    std::cout << "   -ft(code)               用 clang 格式化 cpp 文件代码(cpp/c)" << std::endl;
...
static void show_mail()
{
    ...
    if (!file_mail.is_open())
    {
        ...
        outmail << "-新闻:" << std::endl;
        outmail << "   1. 我们添加了 \"opin\"，你可以在 YunaWrite 环境中运行 .bat/.sh/.command 文件!" << std::endl;
        outmail << "   2. 修改了 Python 插件，现在可以更稳定地运行" << std::endl;
        outmail << "   3. 添加了贡献者列表，我们会记住任何贡献者:)" << std::endl;
        outmail << "   4. \"ft\" 可以通过 clang 格式化 cpp/c 文件" << std::endl;
        outmail << "   5. \"opin\" 可以在本程序中运行 .sh/.bat/.command 文件" << std::endl;
        outmail << "   6. YunaWrite 在 bilibili 上公开" << std::endl;
        outmail.close();
    ...
    while (std::getline(setting_file, line_setting))
    {
        ...
        if (line_num == 0)
        {
            if (line_setting == "blue")
                ...
                fsy.color_setting = "white";
            else if (line_setting == "orange")
                fsy.color_setting = "orange";
            else
...
static void helpListcode()
{
    ...
    std::cout << "  !如需输入中文或表情符号" << std::endl;
    std::cout << "  !请使用 Windows 终端或在 VS 调试器外部运行 CODE" << std::endl;
    std::cout << "  !Ctrl+z(win),Ctrl+d(mac/Linux)   返回/停止写入" << std::endl;
    std::cout << std::endl;
    std::cout << "  *patch             查看程序的补丁历史" << std::endl;
    std::cout << "  *crb               查看 ";
    show_osname();
    std::cout << " 的贡献者" << std::endl;
    std::cout << "  *ctrl+c            退出 ";
    show_osname();
    std::cout << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "  -opexe(插件)        直接打开文件/文件夹" << std::endl;
    std::cout << "  -opin(插件)         在 ";
    show_osname();
    std::cout << " 环境中打开文件，" << std::endl;
    show_osname();
    std::cout << " 环境中运行 .sh/.bat/.command 文件" << std::endl;
    std::cout << "  -ft(插件)          用 clang 格式化 cpp 文件代码(cpp/c)" << std::endl;
    std::cout << "  -mail(插件)         查看邮件信息" << std::endl;
    std::cout << "  -setting(插件)      启用设置" << std::endl;
...
static void Patch()
{
    ...
    show_osname();
    std::cout << " Patch: 0.5.1 About color-orange;color-yello-ft-opin;bilibili" << std::endl;
}
...
static void setting_make(yw_setting& fsy)
{
    ...
    else if (fsy.color_setting == "orange")
    {
...
static void information_os()
{
    ...
    std::cout << "  创建日期(YunaIDE 0.0.1): 2026.8.14" << std::endl;
...
void true_os()
{
    std::cout << "--真(1)" << std::endl;
}

void false_os()
{
    std::cout << "--假(0)" << std::endl;
}
...
void py_os()
{
    ...
    std::cout << "如果你使用 Windows，请直接按回车..." << std::endl;
    std::string pyver;
...
void soft()
{
    ...
    std::cout << "  !如需输入中文或表情符号" << std::endl;
    std::cout << "  !请使用 Windows 终端或在 VS 调试器外部运行 CODE" << std::endl;
    std::cout << "    - OVER_ABOUT_STOP    停止写入" << std::endl;
    std::cout << "    - back               进入 CODE" << std::endl;
    std::cout << "    - quit               退出 ";
    show_osname();
        ...
        try {
            ...
            else if (code == "virfl")
            {
                while (true)
                {
                    ...
                    {
                        show_osname();
                        std::cerr << " 无法处理 \"" << vir_input << "\"" << std::endl;
                    }
                }
            }
            else
            {
                show_osname();
                std::cerr << " 无法处理 \"" << code << "\"" << std::endl;
            }
        }
        catch (std::system_error& a)
        {
            std::cout << "YunaWrite(系统错误):" << a.what() << std::endl;
        }
    }
}